...
<<<<<<< HEAD
     async getCateList() {
       try {
         // 打印请求发送前的URL
         console.log('Sending request to:', 'http://localhost:3000/message4');
     
         const response = await uni.$http.get('http://localhost:3000/message4');
         ...
         if (response.data && response.data.length > 0) {
           ...
           // 为二级分类赋值
           this.cateLevel2 = response.data[0].children;
         } else {
           // 数据为空的处理
